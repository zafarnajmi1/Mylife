...
public final class HybridStorage<T> {
  ...
  public let diskStorage: DiskStorage<T>
   ...
   var storageObservations = [UUID: (HybridStorage, StorageChange) -> Void]()
   var keyObservations = [String: (HybridStorage, KeyChange<T>) -> Void]()
  ...
  public func handleRemovedObject(at path: String) {
...
extension HybridStorage: StorageObservationRegistry {
  ...
  public func notifyStorageObservers(about change: StorageChange) {
...
extension HybridStorage: KeyObservationRegistry {
  ...
  public func notifyObserver(forKey key: String, about change: KeyChange<T>) {
    keyObservations[key]?(self, change)
  }

  public func notifyObserver(about change: KeyChange<T>, whereKey closure: ((String) -> Bool)) {
    let observation = keyObservations.first { key, _ in closure(key) }?.value
    observation?(self, change)
  }

  public func notifyKeyObservers(about change: KeyChange<T>) {
