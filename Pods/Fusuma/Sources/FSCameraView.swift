...
final class FSCameraView: UIView, UIGestureRecognizerDelegate {
    ...
    public var currentDeviceOrientation: UIDeviceOrientation?
    ...
    func initialize() {
        ...
        if let device = device, let _videoInput = try? AVCaptureDeviceInput(device: device) {
            ...
            videoLayer?.videoGravity = AVLayerVideoGravity.resizeAspectFill //AVLayerVideoGravity.resizeAspectFill

            previewViewContainer.layer.addSublayer(videoLayer!)

            session.sessionPreset = AVCaptureSession.Preset.photo//AVCaptureSession.Preset.photo
    ...
    @objc public func handlePinchToZoom(_ pinch: UIPinchGestureRecognizer) {
