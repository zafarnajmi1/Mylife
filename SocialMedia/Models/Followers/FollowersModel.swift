...
//
//	Create by Apple PC on 9/10/2017
...
class FollowersModel : NSObject, NSCoding{

	var data : [FollowersData]!
	var message : String!
	var pagination : FollowersPagination!
	var statusCode : Int!
	...
	init(fromDictionary dictionary: [String:Any]){
		...
		if let dataArray = dictionary["data"] as? [[String:Any]]{
			for dic in dataArray{
				let value = FollowersData(fromDictionary: dic)
				data.append(value)
			}
		}
		message = dictionary["message"] as? String
		if let paginationData = dictionary["pagination"] as? [String:Any]{
			pagination = FollowersPagination(fromDictionary: paginationData)
		}
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         pagination = aDecoder.decodeObject(forKey: "pagination") as? FollowersPagination
